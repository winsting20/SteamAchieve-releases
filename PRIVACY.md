...
버전 1.4 · 2026-09-17 · 앱 버전 0.5.22 기준
...
| `api.github.com` | **새 버전 확인 (매 실행 시 + 실행 중 하루 1회, 항상)** — 릴리스 페이지에 인증 없는 GET 1회/일. 키·SteamID 는 어떤 형태로도 실리지 않음 | 매 실행 시 + 실행 중 하루 1회, 항상 |
...
### 옵트인 기능
- **친구 비교**는 기본 꺼짐이며 설정에서 켠 경우에만 해당 호출이 나갑니다. 끄면 즉시 멈춥니다. **새 버전 확인**은 옵트인이 아니라 매 실행 시 + 실행 중 하루 1회, 항상 나갑니다(위 표 참고).
- **업데이트 받기**는 시작 시 뜨는 업데이트 창 또는 알림에서 [업데이트]를 누를 때만 일어납니다. 받은 파일은 해시가 릴리스의 `SHA256SUMS.txt` 와 일치할 때만 기존 `SteamAchieve.exe` 를 바꾸고, 일치하지 않으면 지웁니다.
