...
모든 데이터는 `%APPDATA%\SteamAchieve\` 안에만 있습니다. 완전히 지우려면 그 폴더를 삭제하면 됩니다. 앱은 레지스트리에 쓰지 않습니다(현재 실행 중인 게임을 표시하려고 Steam 클라이언트 키를 읽기만 합니다). 방금 깬 업적을 바로 보여 주려고 Steam 클라이언트의 본인 업적 통계 캐시 파일도 읽기만 합니다.
